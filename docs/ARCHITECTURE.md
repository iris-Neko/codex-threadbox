...
Codex interface projects are not currently exposed by the public App Server, so the VS Code host does not issue `project/*` requests or present those definitions as manageable projects. Tasks carrying an unknown host-owned `projectId` remain usable, are grouped by working directory, and still open in Codex. Creating a blank task in a Threadbox project opts the VS Code App Server connection into the experimental field allowlist and uses `thread/start({ cwd, historyMode: "legacy" })`, then `thread/name/set`, verifies the stored metadata and legacy history mode with `thread/read`, and finally stores the local root-task assignment. This keeps empty tasks resumable by the official Codex view when newer CLIs default to paginated storage; it does not enable an experimental feature globally or expose additional Webview RPC methods. A history-mode, naming, verification, or assignment failure deletes the new empty task. App Server omits tasks without a first turn from `thread/list`, so the VS Code adapter rehydrates assigned blank tasks by ID with `thread/read` without loading conversation turns.
