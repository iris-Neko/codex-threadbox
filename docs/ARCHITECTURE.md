...
The Webview has a strict nonce CSP, bundled local resources, no Node access, request IDs, timeouts, a fixed method allowlist, and per-method argument validation. The extension rechecks workspace trust before any Codex probe, CLI update, task listing, mutation, or folder-open request. Machine-scoped settings configure the remote CLI path, Codex home, and language. When the probed CLI is outdated, a zero-argument host RPC can stop Threadbox's own App Server, execute only the detected CLI with the official `update` subcommand, and force a fresh minimum-version probe. The Webview cannot supply a command, executable path, or arguments. Remote windows therefore update the remote extension host's CLI; missing and invalid paths are never treated as installation requests.
