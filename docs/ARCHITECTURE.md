...
`ThreadService` pages through active and archived `thread/list` results using all source kinds. It preserves `projectId`, derives the parent/child graph, and accepts unknown protocol response fields. It never reads full conversation turns. Pinning is detected from the installed CLI's generated stable JSON schemas, not its version number. The runtime requires both the list filter and metadata-update boolean fields before sending `isPinned`. Schema generation uses a temporary isolated `CODEX_HOME`; results are cached per executable/version, and malformed, incomplete, or failed detection blocks operations with an explicit error.
...
The extension declares `extensionKind: ["workspace"]`, so Remote SSH, Dev Containers, and Codespaces run the extension, Codex CLI, App Server, and `CODEX_HOME` on the remote host. VS Code is native-sidebar-only: no Manager panel, Webview bundle, or Webview RPC remains. The desktop renderer and shared UI package are retained.

Native commands call the typed host adapter, with workspace trust and runtime argument checks for Codex operations. Scope/archive filters and ordering are validated and saved in workspaceState; search and checked IDs are not persisted. Matching descendants retain their ancestors as context, while Select Filtered excludes context-only ancestors and Trash. Checkbox state is managed explicitly, and filter changes clear checked selection. Workspace scope uses root-task directories, path boundaries, and Windows/UNC case normalization. Fresh inventory and scope confirmation precede batch operations. Native settings remain accessible from the title menu in every environment state. Archive is a sibling of Unassigned, grouped by original root-task working directory and excluding Trash. Checkbox mode is off by default and explicitly enabled in the toolbar; native Ctrl/Shift selection remains available. Right-clicking/dragging a checked item resolves the checked set; unrelated unselected items remain independent. Drag to Archive uses archive operations, while dragging archived tasks to ordinary destinations restores them before assignment, with archive rollback if assignment fails. Row-inline move icons and toolbar-overflow batch actions are removed.
...
The same host-local project file contains a built-in Trash assignment and the previous Threadbox project ID for each trashed root task. A legacy project named `trash` is promoted in place. Moving a task to Trash first applies the normal refreshed running/pinned protection, archives it through App Server, and then writes the local assignment. Restore unarchives the task and returns it to the recorded Threadbox project when it still exists. Either operation rolls back the App Server state if the local write fails. Empty Trash permanently deletes only the roots currently assigned to Trash, using the shared protected deletion path, and never deletes working directories. Trash resolves selected children to their roots, rechecks running/pinned descendants before archiving, and reports partial failures from drag-and-drop or project moves instead of returning a success-shaped snapshot. The sidebar refreshes before displaying results, including partial failures. Writer-lock errors are surfaced as ownership conflicts, not corrupted tasks or permission to force deletion. The sidebar keeps the complete metadata-only error in an output channel, offers navigation through the existing trusted Codex command, and retries only unsuccessful IDs after an explicit click. No lock file is removed. An explicit native-sidebar recovery action on Linux can stop a verified same-user Codex App Server after displaying its process identity, known writer locks, and the warning that ALL sessions served by that backend may be interrupted. A bundled Python 3.9+ helper uses pidfd signals, revalidates executable, UID, process birth time, target lock inode, and approved lock scope before SIGTERM, and requires a second confirmation before SIGKILL. Mismatched procfs PID namespaces, reused PIDs, new executables, expanded lock scopes, untrusted workspaces, configuration changes, and replacement writers stop recovery. Once the lock is released, the normal protected Trash operation runs again. Recovery is restricted to native sidebar actions and is not offered for restore or permanent deletion.

Codex interface projects are not currently exposed by the public App Server, so the VS Code host does not issue `project/*` requests or present those definitions as manageable projects. Tasks carrying an unknown host-owned `projectId` remain usable, are grouped by working directory, and still open in Codex. Creating a blank task in a Threadbox project uses a one-shot App Server connection opted into the experimental field allowlist. It calls `thread/start({ cwd, historyMode: "legacy" })`, then `thread/name/set`, verifies the stored metadata and legacy history mode with `thread/read`, and finally stores the local root-task assignment. It then calls `thread/unsubscribe` and closes that App Server process immediately because the server's normal no-subscriber grace period would otherwise leave the task unavailable to the official Codex view. The sidebar waits for a fresh inventory load after task creation. A history-mode, naming, verification, or assignment failure deletes the new empty task before the one-shot connection closes. App Server omits tasks without a first turn from `thread/list`, so the VS Code adapter rehydrates assigned blank tasks by ID with `thread/read` without loading conversation turns.

Task renaming is an optional host capability, enabled only by the VS Code adapter. Its native command validates the task ID and name, recheck workspace trust, read metadata without turns, call `thread/name/set`, and verify the persisted name. Renaming never resumes a conversation or perform rollback by deleting a task. The sidebar refreshes after success. Codex CLI 0.153.3/0.153.4 cannot rename archived rollouts directly, so archived/Trash entries require an explicit restore first; renaming does not silently unarchive them.
...
Codex CLI `0.153.3` is the minimum supported runtime; older releases are rejected with an upgrade prompt. Each Threadbox release validates the two latest stable Codex CLI versions (`0.153.3` and `0.153.4` for this release), rather than maintaining older compatibility branches. The full blank-task/Trash/restore/empty lifecycle is tested with isolated data. Committed generated protocol files retain their original Codex CLI `0.150.1` provenance; runtime capabilities come from the installed CLI schema, newer unknown response fields remain parseable, and optional capabilities are gated at runtime.
