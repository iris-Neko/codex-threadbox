...
Codex CLI `0.150.0` is the minimum supported runtime; `0.149.x` and older are rejected. Blank-task persistence is validated against `0.150.0`, the affected `0.151.0` release, and the current stable `0.152.0` and `0.152.1` releases. Generated TypeScript protocol files are committed under `src/shared/protocol/generated` from Codex CLI `0.150.1`; the VS Code adapter declares only the newer `historyMode` field it needs, newer unknown response fields remain parseable, and optional capabilities are gated at runtime.
