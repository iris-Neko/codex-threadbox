...
The extension declares `extensionKind: ["workspace"]`. In Remote SSH, Dev Containers, and Codespaces, Codex CLI, `CODEX_HOME`, task data, and the App Server process all remain on the remote host. Configure `threadbox.codexBinary`, `threadbox.codexHome`, or `threadbox.language` as machine-scoped settings when needed. If the detected CLI is below the minimum version, the sidebar and Manager offer **Update Codex CLI**; it runs the official `codex update` self-update command on that extension host and verifies the new version before reconnecting. Missing or invalid CLI paths still require installation or configuration in Settings. An untrusted workspace cannot start Codex or modify task metadata.
...
Threadbox intentionally excludes full transcript viewing/export, task-database repair, unattended CLI upgrades, and a bundled Codex CLI. The VS Code extension can run the official `codex update` command only after the user selects its update action. Recents repair is limited to the disposable desktop sidebar catalog described above.
