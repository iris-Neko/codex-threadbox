...
Run `threadbox` in a TTY for the interactive manager. Script commands are `status`, `list`, `archive`, `unarchive`, `pin`, `unpin`, and `delete`. Use `--json` for the stable `schemaVersion: 1` contract. Non-interactive permanent deletion requires `--yes`.
