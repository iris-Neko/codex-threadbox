...
Open the official Codex sidebar and expand **Threadbox** to organize tasks into host-local projects. The view supports live metadata search, current-workspace import, project creation, creating persistent blank tasks directly inside Threadbox projects, drag-and-drop assignment, multi-select archive/pin/Trash actions, working-directory access, and collapsible spawned tasks. Blank tasks remain visible before their first message even though Codex does not include them in its normal task listing. Set `threadbox.sidebarLocation` to `standalone` to restore a separate Activity Bar container; Threadbox also falls back there when a compatible Codex sidebar is unavailable. Select **Open Threadbox Manager**, or run **Threadbox: Open Manager**, for the full table and advanced filters.
